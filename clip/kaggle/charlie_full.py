...
def gen_flf(a,b):
    ...
    first=last_frame(a); last=load_image(f"{DATA}/{b}.jpg")
    h,w=dims(first); first=first.resize((w,h)); last=last.resize((w,h))
    prompt="anime style, seamless continuous cinematic camera transition between two moments, fluid smooth motion, no cut"
...
for i,(a,b) in enumerate(BRIDGES,1):
    ...
    t0=time.time()
    try:
        r=gen_flf(a,b); ok2+=(r!="skip"); print(f"[B {i}/{len(BRIDGES)}] {a}->{b} {r} {time.time()-t0:.0f}s",flush=True)
